'''
	Converts the Art data provided in XML by Burning Man Org
	to JSON of the PlayaEvents API format.

	Input:
		Burning Man Org Art Placement XML
		PlayaEvents art without location JSON
...
for element in list(xml_input.getroot().findall('{http://www.filemaker.com/fmpdsoresult}ROW')):
	...
	if element.find('{http://www.filemaker.com/fmpdsoresult}T_WWW').text:
		art['description'] 		= element.find('{http://www.filemaker.com/fmpdsoresult}T_WWW').text

	if element.find('{http://www.filemaker.com/fmpdsoresult}T_PublicEmail').text:
		art['contact_email']	= element.find('{http://www.filemaker.com/fmpdsoresult}T_PublicEmail').text

	if element.find('{http://www.filemaker.com/fmpdsoresult}T_URL').text:
		art['url'] 				= element.find('{http://www.filemaker.com/fmpdsoresult}T_URL').text

	if element.find('{http://www.filemaker.com/fmpdsoresult}T_Location').text:
		raw_location 			= element.find('{http://www.filemaker.com/fmpdsoresult}T_Location').text
		proccess_art_loc(art, raw_location)

	if art['name'] in art_name_to_id:
		art['id'] = art_name_to_id[art['name']]
	else:
	...
	if hometown and homestate:
		art['hometown'] = hometown.text.strip() + ', ' + homestate.text.strip()
	elif hometown:
		art['hometown'] = hometown.text.strip()
